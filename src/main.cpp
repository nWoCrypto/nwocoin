...
unsigned int nWorkTargetSpacing = 180;                  // 180 sec block spacing for PoW
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // nwocoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // nwocoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees, const CBlockIndex* pindex)
{
	...
	if(nHeight == 1)
	{
		nSubsidy = 1100000 * COIN;		// 900K ICO + 200K for mixer
		return nSubsidy;
	}
...
{
	int64_t nSubsidy = NORMAL_MINT_PROOF_OF_STAKE_STABLE * nCoinAge / 365;
	return nSubsidy;
}
...
// nwocoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // nwocoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // nwocoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        // nwocoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // nwocoin: track money supply and mint amount info
...
// nwocoin: total coin age spent in transaction, in the unit of coin-days.
...
// nwocoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // nwocoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // nwocoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // nwocoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // nwocoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // nwocoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // nwocoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // nwocoin: getblocks may not obtain the ancestor block rejected
    ...
    // nwocoin: if responsible for sync-checkpoint send it
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "nwocoin: great coin with anonymous wallet!";
        CTransaction txNew;
        txNew.nTime = 1408608435;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1408608435;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 5480307;
        if(fTestNet)
        {
            block.nNonce = 0;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("e49cbbe71300e0d14ba907f3c0045b17d8158c49ebf9b3d4ebbfec6b11be4e1a"));
        block.print();
        ...
        // nwocoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0xda, 0xdb, 0xfc, 0xfe };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // nwocoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // nwocoin: tell downloading node about the latest block if it's
